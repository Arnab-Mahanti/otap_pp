...
namespace rapidcsv
{
  ...
  struct SeparatorParams
  {
     ...
     */
    explicit SeparatorParams(const bool pDelimWhitespace = true, const char pSeparator = ',', const bool pTrim = false,
                             const bool pHasCR = sPlatformHasCR, const bool pQuotedLinebreaks = false,
                             const bool pAutoQuote = true, const char pQuoteChar = '"')
        : mDelimWhitespace(pDelimWhitespace), mSeparator(pSeparator), mTrim(pTrim), mHasCR(pHasCR), mQuotedLinebreaks(pQuotedLinebreaks), mAutoQuote(pAutoQuote), mQuoteChar(pQuoteChar)
    {
    }

    bool mDelimWhitespace;
    /**
  ...
  struct LineReaderParams
  {
    ...
    explicit LineReaderParams(const bool pSkipCommentLines = true,
                              ...
                              const bool pSkipEmptyLines = true)
    ...
    void ParseCsv(std::istream &pStream, std::streamsize p_FileLength)
    {
      ...
      while (p_FileLength > 0)
      {
        ...
        for (size_t i = 0; i < static_cast<size_t>(readLength); ++i)
        {
          ...
          else if (buffer[i] == mSeparatorParams.mSeparator && !isspace(mSeparatorParams.mSeparator))
          {
            if (!quoted)
            {
          ...
          else if (isspace(buffer[i]) && mSeparatorParams.mDelimWhitespace)
          {
            if (!wspace && !cell.empty())
            {
